...
def parse_args():
    parser = argparse.ArgumentParser(description="Weather-aware training script")
...
def main():
    ...
    # Learning rate scheduling: default to Cosine Annealing, eta_min can be overridden via min_lr config
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=int(cfg.get("epochs", 30)),
    ...
    try:
        for epoch in range(1, epochs + 1):
            if epoch > stage_end_epoch and stage_ptr + 1 < len(curriculum):
                ...
                print(f"Entering {curriculum[stage_ptr]['name']}, weathers={curriculum[stage_ptr]['weathers']}")
            ...
            print(
                f"Epoch [{epoch}/{epochs}] - stage={current_stage} loss={train_loss:.4f} val mIoU={val_miou:.4f}{per_weather_str}",
                flush=True,
            )
            if writer:
